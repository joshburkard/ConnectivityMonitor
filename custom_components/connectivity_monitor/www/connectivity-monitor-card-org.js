(() => {
  if (customElements.get("connectivity-monitor-card")) return;
  ...
  class ConnectivityMonitorCard extends HTMLElement {
    ...
    _styles() {
      return `
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-subtitle { font-size: 0.85rem; color: var(--secondary-text-color); margin-top: 4px; }
        .configure-btn {
          display: flex;
          align-items: center;
          gap: 6px;
        ...
        .tab-bar {
          ...
          margin-top: 12px;
          margin-bottom: 16px;
          border-bottom: 2px solid var(--divider-color, rgba(0,0,0,.12));
        }
        .tab-btn {
          background: none;
          border: none;
        ...
        .device-card {
          ...
          margin-bottom: 4px;
          box-shadow: var(--ha-card-box-shadow, 0 1px 4px rgba(0,0,0,.08));
          overflow: hidden;
    ...
    _render() {
      ...
      try {
        ...
        this.shadowRoot.innerHTML =
          ...
          "<ha-card>" +
            "<div class=\"card-header\">" +
              "<div>" +
                "<div class=\"card-title\">Connectivity Monitor</div>" +
                "<div class=\"card-subtitle\">Monitoring " + esc(totalDevices) + " device" + (totalDevices !== 1 ? "s" : "") + "</div>" +
              "</div>" +
        ...
        this.shadowRoot.innerHTML =
          "<style>" + this._styles() + "</style>" +
          "<ha-card><div class=\"error-msg\">Card error: " + esc(String(err)) + "</div></ha-card>";
      }
    }
  }
  ...
  window.customCards.push({
    ...
    description: "Displays network, ZigBee, Matter and ESPHome device connectivity status.",
    preview: false,
  });
})();
